  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between text-xs font-semibold text-fw-text">
        <span className="tabular-nums">{isPercent ? `${home}%` : home}</span>
        <span className="text-[10px] font-bold uppercase tracking-wider text-fw-text-faint">{label}</span>
        <span className="tabular-nums">{isPercent ? `${away}%` : away}</span>
      </div>
...
export default function MatchResultCard({
  ...
  return (
    <div className="fw-animate-pop overflow-hidden rounded-2xl border border-fw-border bg-fw-bg-elevated">
      ...
      >
        <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-fw-text-faint">{t("match.fullTime")}</p>
        ...
        >
          {outcome === "win" ? t("match.victory") : outcome === "loss" ? t("match.defeat") : t("match.draw")}
        </p>
      </div>

      <div className="grid grid-cols-1 gap-6 p-5 sm:grid-cols-2">
        <div>
          <p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-fw-text-faint">{t("match.goals")}</p>
          {result.goals.length === 0 ? (
            <p className="text-sm text-fw-text-faint">{t("match.noGoals")}</p>
          ) : (
        ...
        <div className="flex flex-col gap-2.5">
          <p className="mb-0.5 text-[11px] font-bold uppercase tracking-wider text-fw-text-faint">{t("match.stats")}</p>
          <StatRow label={t("match.possession")} home={result.homeStats.possession} away={result.awayStats.possession} isPercent />
          <StatRow label={t("match.shots")} home={result.homeStats.shots} away={result.awayStats.shots} />
          <StatRow label={t("match.onTarget")} home={result.homeStats.shotsOnTarget} away={result.awayStats.shotsOnTarget} />
          <StatRow label={t("match.corners")} home={result.homeStats.corners} away={result.awayStats.corners} />
        </div>
      </div>

      <div className="border-t border-fw-border p-5">
        <p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-fw-text-faint">{t("match.playerRatings")}</p>
        <ul className="space-y-1.5">
          {topRatings.map((r) => (

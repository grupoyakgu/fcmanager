    ...
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between gap-3 border-b border-fw-border bg-fw-bg/90 px-4 backdrop-blur lg:px-8">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-fw-text-faint">{t("topbar.matchday", { n: currentWeek })}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        <div className="hidden flex-col items-end sm:flex">
          ...
          <span className="text-[10px] font-semibold uppercase tracking-wider text-fw-text-faint">{t("topbar.cash")}</span>
        </div>
        {position !== null && (
          <div className="hidden flex-col items-end sm:flex">
            <span className="font-display text-sm font-bold tabular-nums text-fw-text">#{position}</span>
            <span className="text-[10px] font-semibold uppercase tracking-wider text-fw-text-faint">{t("topbar.leagueShort")}</span>
          </div>
        )}
        <LanguageToggle />
        <NotificationBell />
